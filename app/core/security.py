...
`Tenant` is the SaaS auth boundary (see `app.db.models.Tenant`) - a single
...
against that `tenant_id` (see `app.services.tenancy`) before being
...
from app.db.models import ApiKey, Role
...
class AuthContext:
    ...
    via `app.services.tenancy` before being trusted.
    """
    tenant_id: UUID
    role: Role
    api_key_id: UUID
