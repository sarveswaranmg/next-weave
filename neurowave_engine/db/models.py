...
class Role(str, enum.Enum):
    ...
    rather than in `neurowave_engine.core.security` so `neurowave_engine.core.security` can import
    models normally (ApiKey, Role) without creating a circular import -
    `neurowave_engine.db.models` must not depend on `neurowave_engine.core.security`.
    """
    ADMIN = "admin"
    DEVELOPER = "developer"
    READONLY = "readonly"
...
class ApiKey(Base):
    ...
    `neurowave_engine.services.tenancy`), which is what stops one tenant from reading or
