...
class CognitiveAnalyzer:
    ...
    def __init__(self, model: str = "gpt-4"):
        """Initialize analyzer with OpenAI model. The client is constructed
        lazily (not at module import time) so importing this module never
        requires OPENAI_API_KEY to be set - only actually calling
        analyze_memory() does. The default heuristic scoring path
        (score_memory(..., use_llm=False)) never touches this class at all."""
        self.model = model
